...
```{r}
#world maop by number of players per country? (not done yet)
world_map <- map_data("world")
world_map <- subset(world_map, region != "Antarctica")
...
bp_winners_cluster |>
  mutate(
    receive_clusters = as.factor(nstart_kmeans$cluster)
  ) |>
  ggplot(aes(x = avg_ace_lost, y = w_bp_percent_avg,
             color = receive_clusters)) +
  geom_point(size = 4) +
  ggthemes::scale_color_colorblind() +
  theme(legend.position = "bottom") +
  coord_fixed() +
  labs(
      title="Clustering of Winning Players using Aces Lost to and Break Points Saved",
      y= "Average Break Point Save Percentage per Game",
...
bp_losers_cluster |>
  ...
  coord_fixed()+
  labs(
    title="Clustering of Losing Players using Aces Lost to and Break Points Saved",
    y= "Average Break Point Save Percentage per Game",
    x= "Average Aces lost to per Game"
  )
